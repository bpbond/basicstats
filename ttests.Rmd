...
**We also never can observe or measure _all_ the 'elements of interest'.**
By definition, populations are large are frequently inaccessible due to time,
money, logistics, etc. And we _sample_ from the population.
...
It has the same units as our original data, and indicates how close the data 
...
By taking the square root, we return the measure to the same scale as the mean. It indicates how close the data is to the mean.
...
ggplot(dat, aes(x, y)) + geom_point() + facet_wrap(~s) +
...
Watch how the distribution (density) of the data change as sample size increases,
getting closer and closer to the theoretical _normal distribution_ (dashed line):
...
* the alternative (or _null_) hypothesis: "true difference in means is not equal to 0"
* the pooled _degrees of freedom_ (closely related to the sample size)
* the _p-value_. Here it is 0.079, which traditionally would be interpreted as _weak evidence_ against the null hypothesis
...
This means that we can use a **paired t-test**:

```{r}
t.test(group1, group2, paired = TRUE)
```

This is _strong evidence_ against the null hypothesis.

The pairing of samples reduces inter-subject variability (because it makes 
comparisons between the same subject), and thus is more powerful than
the unpaired t-test.
